...
# 遍历每个 r_bm 文件夹
for rbm_folder in "$base_dir"/r_bm_*; do
  if [ -d "$rbm_folder" ]; then
    rbm=$(basename "$rbm_folder")
    
    # 遍历每个日期文件夹
    for date_folder in "$rbm_folder"/*; do
      if [ -d "$date_folder" ]; then
        # 获取日期文件夹的基本名称，即日期
        date=$(basename "$date_folder")

        # 创建一个针对每个日期的合并命令文件，存放在中心目录中
        merge_script="${central_dir}/merge_commands-${rbm}-${date}.sh"
        : > "$merge_script"  # 清空或创建新文件
        chmod +x "$merge_script"  # 给予执行权限

        # 定义每个日期文件夹中的合并后文件，同样存放在中心目录中
        date_merged_file="${central_dir}/obvs_coalHadrons-${rbm}-${date}.root"
...
for rbm_folder in "$base_dir"/r_bm_*; do
  if [ -d "$rbm_folder" ]; then
    rbm=$(basename "$rbm_folder")
    all_merged_files=$(find "$central_dir" -name "obvs_coalHadrons-${rbm}-*.root")
    final_merged_file="${central_dir}/obvs_coalHadrons-${rbm}-FinalMerged.root"

    if [ -n "$all_merged_files" ]; then
      hadd -f "$final_merged_file" $all_merged_files
      echo "All observation result files for ${rbm} have been merged into $final_merged_file"
    else
      echo "No files to merge for ${rbm}."
    fi
  fi
done