...
inputFile=${line}
outputFile=./${date_folder}/dataCoalHadrons-${seq_number}.root
obvsFile=./${date_folder}/obvsCoalHadrons-${seq_number}.root
EOL

        echo "Created config file: $conf_file in $date_folder"
...
EOL
    ...
    chmod 755 "$date_folder/condor.sh"
    echo "Created and set permissions for condor.sh in $date_folder"

    # 创建 condor.con 文件
    cat > "$date_folder/condor.con" << EOL
Universe     = vanilla
Notification = Error
Initialdir   = ./
...
EOL
    ...
    echo "Created condor.con in $date_folder"

    # 添加提交命令到submit.sh
    echo "condor_submit $date_folder/condor.con" >> submit.sh
